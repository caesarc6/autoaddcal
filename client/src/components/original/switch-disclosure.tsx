...
export interface SwitchDisclosureProps {
  title?: string;
...
export function SwitchDisclosure({
  ...
  const handleSubOptionToggle = (event: MouseEvent) => {
  ...
  return (
    <MotionConfig transition={springConfig}>
      <motion.div
        layout
        initial={false}
        className={`w-full max-w-[340px] overflow-hidden rounded-[35px] bg-transparent p-[5px] transition-colors duration-300 will-change-transform ${
          isEnabled
      ...
      >
        <motion.div
          layout
          className="flex flex-col overflow-hidden rounded-[35px] bg-[#F6F5FA] shadow-sm dark:bg-neutral-800"
        >
          <motion.div
            layout
          ...
          >
            <div className="flex items-center gap-3">
              <div className="rounded-full p-2 text-[#ADACB8] dark:text-neutral-500">
                <RiBubbleChartFill size={18} />
              </div>
              <span className="text-sm font-medium text-[#28272A] dark:text-neutral-100">
                {title}
              </span>
            </div>
          ...
          <AnimatePresence initial={false}>
            {isEnabled && showSubOption && subOptionLabel ? (
              <motion.div
                key="sub-option"
              ...
              >
                <motion.div
                  ...
                  className="group flex cursor-pointer items-center gap-3 px-4 pb-4"
                ...
                >
                  ...
                  <span className="text-sm text-[#28272A] dark:text-neutral-200">
                    {subOptionLabel}
                  </span>
                </motion.div>
              </motion.div>
